...
func (d *mackerelServiceMetadataDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	serviceName, namespace := data.ServiceName.ValueString(), data.Namespace.ValueString()
	remoteData, err := mackerel.ReadServiceMetadata(ctx, d.Client, serviceName, namespace)
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("Unable to read Service Metadata: %s/%s", serviceName, namespace),
			err.Error(),
		)
		return
	}
